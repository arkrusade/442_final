    ...
    for _, _, files in os.walk(args.data_dir):
        ...
        for file in files:
            ...
            data, labels = txt_to_npy(all_file_count, index, data, labels,
                                      ...
                                      args.one_hot)
            findex = int(index / batch_size) - 1
            if index % batch_size == 0:
                data = np.array(data, dtype=np.float32)
                labels = np.array(labels)
                out_data = save_dir + '/phd08_data_' + str(findex)
                out_labels = save_dir + '/phd08_labels_' + str(findex)
                np.save(out_data, data)
                np.save(out_labels, labels)

                last = last + data.shape[0]
                bounds.append(last)

                data = []
                labels = []
                print("  FILE_SAVED:: filename : " + save_dir + '/phd08_data_' + str(findex))

    import pdb; pdb.set_trace()  # XXX BREAKPOINT
    np.save(save_dir + '/header.npy', np.array(bounds))
